...
deleteBy p x (y:ys) = 
  if p x y 
     then ys 
     else x : deleteBy p x ys

groupBy :: (a -> a -> Bool) -> [a] -> [[a]]
groupBy p [] = []
groupBy p [x] = [[x]]
groupBy p (x:xs) = 
  let (ys:yss) = groupBy p xs
      in if p x (head ys)
            then (x:ys) : yss
            else [x] : ys : yss
...
elemIndices' n x [] = []
elemIndices' n x (y:ys) = 
  if x == y
     then n : elemIndices' (n+1) x ys 
     else elemIndices' (n+1) x ys
-- elemIndices' n x (y:ys) = 
--  (if x == y then (n :) else id) (elemIndices' (n+1) x ys)
...
-- elemIndices x [] = []
-- elemIndices x (y:ys) =  
--  (if x == y then (0 :) else id) $ map (+1) $ elemIndices' x ys

-- elemIndices x = 
--   map fst . filter ((==x) . snd) . zip [0..]
...
---------------------------------------------
--- más definiciones para pesnsar mucho -----
---------------------------------------------
